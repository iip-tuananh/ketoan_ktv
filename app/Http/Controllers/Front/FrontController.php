...
class FrontController extends Controller
{
    ...
    public function homePage() {
        $data['services'] = Service::query()->where(['show_home_page' => 1, 'status' => 1])->with('image')->latest()->get();
        $data['posts'] = Post::query()->where(['pin' => 1, 'status' => 1])->with('image')->latest()->get()->take(3);
        $data['partners'] = Partner::query()->with('image')->latest()->get();
        $data['reviews'] = Review::query()->with('image')->latest()->get();
        $data['banners'] = Banner::query()->with('image')->orderBy('position')->get();
    ...
    public function about_page(Request $request) {
        $about = AboutPage::query()->with(['image', 'galleries.image'])->find(1);
        $partners = Partner::query()->with('image')->latest()->get();

        return view('site.about_page', compact( 'about', 'partners'));
    }

    public function teams(Request $request, $id = null) {
        if(! $id) {
            $banner = BannerPage::query()->with(['image'])->find(3);
            $teams = Team::query()->with(['image'])->latest()->get();

            return view('site.teams', compact( 'banner', 'teams'));
        } else {
            $banner = BannerPage::query()->with(['image'])->find(3);
            $team = Team::query()->with(['image', 'galleries.image'])->find($id);

            return view('site.team_detail', compact( 'banner', 'team'));
        }

    }

    public function services(Request $request, $slug = null) {
       $categoryService = PostCategory::query()->with('image')->where('slug', $slug)->firstOrFail();
    ...
    public function getServices(Request $request) {
        $services = Service::query()->with(['category', 'image', 'galleries.image'])
            ->where('status', 1)
            ->latest()->paginate(6);
    ...
    public function getServiceDetail(Request $request, $slug) {
        $service = Service::query()->with(['category', 'image', 'galleries.image'])->where('slug', $slug)->firstOrFail();
        ...
        return view('site.service_detail', compact('service', 'otherServices', 'postCategory'));
    }

    public function getPostCategory(Request $request, $slug) {
        $postCate = PostCategory::query()->where('slug', $slug)->firstOrFail();
        $posts = Post::query()->with(['category', 'image'])->where('cate_id', $postCate->id)
            ->where('status', 1)->paginate(9);
        $postCategory = PostCategory::query()
            ->where('type', 1)->orderBy('sort_order')->get();

        return view('site.blog', compact('posts', 'postCate', 'postCategory'));
    }

    public function getPostDetail(Request $request, $slug) {
        $post = Post::query()->with(['category', 'image'])->where('slug', $slug)->first();
        $otherPosts = Post::query()->whereNotIn('id', [$post->id])
            ->where('status', 1)
    ...
    public function getCourseDetail(Request $request, $slug) {
        $course = Course::query()->with(['image', 'image_back'])->where('slug', $slug)->firstOrFail();
        $otherCourses = Course::query()
            ->where('status', 1)
            ->latest()->get();

        return view('site.course_detail', compact('course', 'otherCourses'));
    }

    public function getListProduct(Request $request, $slug = null) {
        $category = null;
        $allCates = Category::query()->withCount(['products' => function ($query) {
        ...
        if(! $slug) {
            $products = Product::query()->with(['category', 'image'])
                ...
                ->paginate(15)->appends($request->only('sort'));;
        } else {
        ...
        return view('site.product_list', compact('products', 'category', 'allCates'));
    }
    public function getProductDetail($slug) {
        $product = Product::query()->with(['category.image', 'image', 'galleries.image'])->where('slug', $slug)->firstOrFail();
        $otherProducts = Product::query()->with(['image', 'image_back'])
            ->where('cate_id', $product->cate_id)
            ->whereNotIn('id', [$product->id])->latest()->get()->take(4);
    ...
    public function blogs(Request $request, $slug = null) {
        ...
        $othersPost = Post::query()->with(['image'])
            ->where('status', 1)->latest()->take(3)->get();

        return view('site.blog', compact('blogs', 'categoryBlog', 'categories', 'othersPost'));
    }
    ...
    public function blogDetail(Request $request, $slug) {
        ...
        $otherBlogs = Post::query()->with('image')
            ...
            ->where('status', 1)
            ->latest()->get();
        $categoryBlog = PostCategory::query()->with('image')->where('id', $blog->cate_id)->firstOrFail();
        $categories = PostCategory::query()->withCount('posts')->where('type', 1)->get();
    ...
    public function searchs(Request $request) {
        ...
        $services = Service::query()->where('status', 1)
            ->where('name', 'like', '%'.$keyword.'%')->latest()->get();
        $posts = Post::query()->where('status', 1)
            ->where('name', 'like', '%'.$keyword.'%')->latest()->get();
        $postCategory = PostCategory::query()
            ->where('type', 1)->orderBy('sort_order')->get();

        return view('site.search', compact('services','posts', 'keyword', 'postCategory'));
    }
